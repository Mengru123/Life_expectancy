-- there are few mistakes in g_ct2rls, the correct one is downloaded as dim.ct_rlsreviewed
-- detailed info can be found:https://jira.mchi.mcgill.ca/browse/MPH-1542
...
select count(midyear) from LE_table1;
...
create table public.LE_table_denom as
  (select
     ...
     tem.midyear
   from public.ct_rlsreviewed as ct
     right join
     (SELECT
...
right join public.ct_rlsreviewed g
  on len.fk_ct = g.src
